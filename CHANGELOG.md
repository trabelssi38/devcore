...
### Fixed
- **Correction des Arguments CLI Engine (`launch`)** : Ajout du support de l'argument `-QuickStart` (ainsi que `--quickstart` / `--quick-start`) dans `devcore_engine/cli.py` pour garantir la rétro-compatibilité totale des appels `launch.ps1 -QuickStart` lors du démarrage de session (`session_start.ps1`).

### Added / Documentation
- **Clarification du Routage de Compression Headroom & 9Router** : Documentation de la chaîne d'optimisation de jetons (`8788/8787` -> `20128`) pour Claude Code Desktop, garantissant que la compression s'applique en amont du multiplexeur local 9Router.
