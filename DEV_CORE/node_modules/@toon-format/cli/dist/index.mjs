...
//#endregion
//#region ../toon/src/decode/errors.ts
/**
...
var package_default = {
	...
	bugs: { "url": "https://github.com/toon-format/toon/issues" },
	sideEffects: false,
	exports: { ".": {
...
async function decodeToJson(config) {
	if (config.expandPaths === "safe") await writeStreamingJson(jsonStringifyLines(decode(await readInput(config.input), {
		indent: config.indent,
		strict: config.strict,
		expandPaths: config.expandPaths
	}), config.indent), config.output);
	else await writeStreamingJson(jsonStreamFromEvents(decodeStream(readLinesFromSource(config.input), {
		indent: config.indent,
		strict: config.strict
	}), config.indent), config.output);
	if (config.output) {
...
function formatError(error, options) {
	const sections = [];
	if (error instanceof ToonDecodeError && error.line !== void 0) sections.push(formatDecodeError(error));
	else sections.push(String(error));
