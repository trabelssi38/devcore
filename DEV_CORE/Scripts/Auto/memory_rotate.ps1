...
if (-not (Test-Path $memPath)) {
    # Créer MEMORY.md initial
    @"
# MEMORY.md — DEV_CORE v6
<!-- Auto-géré par memory_rotate.ps1 -->
<!-- Score min inclusion : 0.5 | Max entrées : 50 -->
...
"@ | Set-Content $memPath -Encoding UTF8
    Log "MEMORY.md créé" "Green"; exit 0
}
...
if ($lines -gt 300) {
    ...
    Log "Archivé : $archDir\MEMORY_$TODAY.md" "Green"
    # Garder les 200 premières lignes (top entries)
    Get-Content $memPath | Select-Object -First 200 | Set-Content $memPath -Encoding UTF8
    Log "MEMORY.md tronqué à 200 lignes" "Cyan"
}
