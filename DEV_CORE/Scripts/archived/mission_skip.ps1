...
Write-Host "  ⚠ Skip $($current.id) — $($current.title)" -ForegroundColor Yellow
$c = Read-Host "  Confirmer le skip ? (o/N)"
if ($c -notmatch "^o|^y") { exit 0 }
$current.status = "skipped"
$board | ConvertTo-Json -Depth 10 | Set-Content $mFile -Encoding UTF8
Write-Host "  → Skipped. dc next mission pour continuer." -ForegroundColor DarkGray
